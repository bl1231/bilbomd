...
PDB_FILE="$SCRIPT_DIR/data/big-c3befbc/c3befbc.pdb"
DAT_FILE="$SCRIPT_DIR/data/big-c3befbc/c3befbc.dat"
PAE_FILE="$SCRIPT_DIR/data/big-c3befbc/c3befbc-pae.json"
...
HTTP_STATUS=$(curl -# -o "$RESPONSE_FILE" -w "%{http_code}" \
  ...
  -F "bilbomd_mode=auto" \
  -F "md_engine=CHARMM" \
  -F "title=${TITLE_DATE}-c3befbc-charmm-${TITLE_SUFFIX}" \
  -F "pdb_file=@${PDB_FILE}" \
  -F "dat_file=@${DAT_FILE}" \
  -F "pae_file=@${PAE_FILE}" )
