...
PDB_FILE="$SCRIPT_DIR/data/sans/sans.pdb"
DAT_FILE="$SCRIPT_DIR/data/sans/sans-data.dat"
INP_FILE="$SCRIPT_DIR/data/sans/const.inp"
...
HTTP_STATUS=$(curl -# -o "$RESPONSE_FILE" -w "%{http_code}" \
  ...
  -F "bilbomd_mode=sans" \
  -F "title=${TITLE_DATE}-api-test-sans-${TITLE_SUFFIX}" \
  -F "pdb_file=@${PDB_FILE}" \
  -F "dat_file=@${DAT_FILE}" \
  -F "inp_file=@${INP_FILE}" )
