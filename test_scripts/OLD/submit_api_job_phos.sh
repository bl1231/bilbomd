...
PDB_FILE="$SCRIPT_DIR/data/phos/phos.pdb"
DAT_FILE="$SCRIPT_DIR/data/phos/saxs-data.dat"
INP_FILE="$SCRIPT_DIR/data/phos/const.inp"
...
HTTP_STATUS=$(curl -# -o "$RESPONSE_FILE" -w "%{http_code}" \
  ...
  -F "bilbomd_mode=pdb" \
  -F "title=${TITLE_DATE}-api-test-phos-${TITLE_SUFFIX}" \
  -F "pdb_file=@${PDB_FILE}" \
  -F "dat_file=@${DAT_FILE}" \
  -F "inp_file=@${INP_FILE}" )
