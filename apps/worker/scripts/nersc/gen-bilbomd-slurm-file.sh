...
# Docker images
WORKER=bilbomd/bilbomd-perlmutter-worker:0.0.27
...
generate_pdb2crd_input_files() {
    echo "Generating pdb2crd input files. Using docker image: ${WORKER}"
...
srun --job-name af-pdb2crd podman-hpc run --network slirp4netns --rm -v \${WORKDIR}:/bilbomd/work ${WORKER} /bin/bash -c "cd /bilbomd/work/ && python /app/scripts/pdb2crd.py af-rank1.pdb . > pdb2crd_output.txt"
...
while IFS= read -r filename; do
    ...
    srun --ntasks=1 --cpus-per-task=\$cpus --cpu-bind=cores --job-name pdb2crd podman-hpc run --network slirp4netns --rm -v \${WORKDIR}:/bilbomd/work ${WORKER} /bin/bash -c "cd /bilbomd/work/ && charmm -o \${filename_prefix}.out -i \${filename}" &
...
srun --ntasks=1 --cpus-per-task=$NUM_CORES --cpu-bind=cores --job-name meld podman-hpc run --network slirp4netns --rm -v \${WORKDIR}:/bilbomd/work ${WORKER} /bin/bash -c "cd /bilbomd/work/ && charmm -o pdb2crd_charmm_meld.out -i pdb2crd_charmm_meld.inp"
MELD_EXIT=\$?
...
EOF
    ...
    for inp in "${g_pdb2crd_inp_files[@]}"; do
        ...
        local command="srun --ntasks=1 --cpus-per-task=$cpus --cpu-bind=cores --job-name pdb2crd podman-hpc run --network slirp4netns --rm -v ${WORKDIR}:/bilbomd/work ${WORKER} /bin/bash -c \"cd /bilbomd/work/ && charmm -o ${inp%.inp}.out -i ${inp}\" &"
    ...
    local command="srun --ntasks=1 --cpus-per-task=$NUM_CORES --cpu-bind=cores --job-name meld podman-hpc run --network slirp4netns --rm -v ${WORKDIR}:/bilbomd/work ${WORKER} /bin/bash -c \"cd /bilbomd/work/ && charmm -o pdb2crd_charmm_meld.out -i pdb2crd_charmm_meld.inp\""
    echo $command >> $WORKDIR/pdb2crd
...
srun --ntasks=1 --cpus-per-task=$NUM_CORES --cpu-bind=cores --job-name pae2const podman-hpc run --network slirp4netns --rm -v \${WORKDIR}:/bilbomd/work ${WORKER} /bin/bash -c "cd /bilbomd/work/ && python /app/scripts/pae2const.py ${pae_file} --crd_file ${in_crd_file} --openmm-const-file constraints.yaml > pae2const.log 2>&1"
PAE_EXIT=\$?
...
srun --ntasks=1 --cpus-per-task=$NUM_CORES --cpu-bind=cores --job-name minimize podman-hpc run --network slirp4netns --rm -v \${WORKDIR}:/bilbomd/work ${WORKER} /bin/bash -c "cd /bilbomd/work/ && charmm -o minimize.out -i minimize.inp"
MIN_EXIT=\$?
...
srun --ntasks=1 --cpus-per-task=$NUM_CORES --cpu-bind=cores --job-name initfoxs podman-hpc run --network slirp4netns --rm -v \${WORKDIR}:/bilbomd/work ${WORKER} /bin/bash -c "cd /bilbomd/work/ && foxs ${foxs_args[@]} > initial_foxs_analysis.log 2> initial_foxs_analysis_error.log"
INITFOXS_EXIT=\$?
...
srun --ntasks=1 --cpus-per-task=$NUM_CORES --cpu-bind=cores --job-name heat podman-hpc run --network slirp4netns --rm -v \${WORKDIR}:/bilbomd/work ${WORKER} /bin/bash -c "cd /bilbomd/work/ && charmm -o heat.out -i heat.inp"
HEAT_EXIT=\$?
...
srun --gpus=4 --job-name alphafold podman-hpc run --network slirp4netns --rm --gpu -v \${WORKDIR}:/bilbomd/work ${AF_WORKER} /bin/bash -c "cd /bilbomd/work/ && colabfold_batch --num-models=3 --amber --use-gpu-relax --num-recycle=4 af-entities.fasta alphafold"
AF_EXIT=\$?
...
EOF
    ...
    for inp in "${g_md_inp_files[@]}"; do
        local command="srun --ntasks=1 --cpus-per-task=$cpus --cpu-bind=cores --job-name md$count podman-hpc run --network slirp4netns --gpu --rm -v \${WORKDIR}:/bilbomd/work ${WORKER} /bin/bash -c \"cd /bilbomd/work/ && charmm -o ${inp%.inp}.out -i ${inp}\" &"
...
srun --ntasks=1 --cpus-per-task=$NUM_CORES --cpu-bind=cores --job-name dcd2pdb podman-hpc run --network slirp4netns --rm -v \${WORKDIR}:/bilbomd/work ${WORKER} /bin/bash -c "cd /bilbomd/work/ && ./run_dcd2pdb.sh"
DCD2PDB_EXIT=\$?
...
srun --ntasks=1 --cpus-per-task=$NUM_CORES --cpu-bind=cores --job-name foxs podman-hpc run --network slirp4netns --rm -v \${WORKDIR}:/bilbomd/work ${WORKER} /bin/bash -c "cd /bilbomd/work/foxs && ../run_foxs.sh"
FOXS_EXIT=\$?
...
srun --ntasks=1 --cpus-per-task=$NUM_CORES --cpu-bind=cores --job-name multifoxs podman-hpc run --network slirp4netns --rm -v \${WORKDIR}:/bilbomd/work ${WORKER} /bin/bash -c "cd /bilbomd/work/ && ./run_multifoxs.sh"
MFOXS_EXIT=\$?
