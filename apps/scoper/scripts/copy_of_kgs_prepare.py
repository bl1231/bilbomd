...
class PDBFile:
    ...
    def getHydrogenBonds(self, threshold=-1.0):
        ...
        """
        bonds = []
        print(f"Identifying hbonds at energy cutoff {threshold}") 
        ...
        for atom in self.atoms:
            ...
            if atom.resi != currentResId or ((atom.name, atom.alt) in resList) : #New residue or wrong numbering
                ...
                if atom.resi > currentResId + 1: #This is a gap
                    print(f"Chain {str(currentChain)}: residue gap between {str(currentResId)} and {str(atom.resi)}")
                    ...
                    if ( (atom.name, atom.alt) in resList): #New residue (even if same wrong number, or same name)
                        if inconsistencyFlag == 0:
                            ...
                            print(f"Inconsistent residue name at {(atom.resi)+atom.resn} in chain {str(currentChain)}")
                            inconsistencyFlag = 1
                        currentResId += 1
...
if __name__ == "__main__":
    ...
    if "-alt" in argv:
        ...
        argv.remove("-alt")
        argv.remove(altloc)
        print(f"Keeping alternate location {altloc}") 
    ...
    if "-chain" in argv:
        ...
        print(f"Keeping only chain {chainID}")
    ...
    if "-energy" in argv:
        ...
        print(f"Minimum h-bond energy {cutoff}") 
        cutoff = float(cutoff)
    ...
    if "-cutoffD" in argv:
        ...
        print(f"Hydrophobic distance cutoff {cutoffD}")
    ...
    if "-model" in argv:
        ...
        print(f"Keeping only model {model}")
        model = int(model)
