...
# Prefer a shared fast implementation from tools/python/guinier.py.
try:
    from guinier import guinier_scan  # type: ignore
except Exception:
    raise ImportError(
        "Failed to import guinier_scan from guinier.py. Ensure tools/python is in PYTHONPATH."
    )
...
def estimate_mw(
    ...
    guinier_results = guinier_scan(
        q, intensity, sigma, min_points=10, qrg_min=0.30, qrg_max=1.30, r2_floor=0.95
    )
    print(f"Guinier results: {guinier_results}")
    I0, Rg = guinier_results["I0"], guinier_results["Rg"]
